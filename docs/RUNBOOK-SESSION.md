...
| `need_help` | Quiz sospeso (domande da risolvere) | AI: WebSearch → `answers-cli resolve` (auto-share e riapertura conservativa) → start. |
| `off_hours` | Scheduler vivo, browser fermo fuori turno | Nessuna azione: heartbeat e prossimo turno restano aggiornati. |
| `preflight_failed` | Fixture/selettori non coerenti | Aggiorna col curl; il browser non è stato aperto e nessun quiz è stato toccato. |
