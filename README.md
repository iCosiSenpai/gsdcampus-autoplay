...
> 🔧 **Segnalazione bug al manutentore (opt-in).** Quando l'AI non riesce a risolvere un problema in loco, può aprire in automatico una *issue* sulla repo pubblica del manutentore (GitHub) invece di "rompere" il codice. Disattivata di default: per attivarla, rispondi "sì" al prompt del setup e incolla un token "issue-reporter" (fine-grained PAT GitHub, scope Issues:write, solo la repo del progetto — te lo fornisce il manutentore). Il token resta nel tuo `config.json` (mai pubblicato). I dati sensibili (CF, token di sessione, cookie) sono redatti prima dell'invio, e l'AI ti mostra sempre il draft e chiede conferma prima di spedire.
