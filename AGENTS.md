...
- **Sul Mac del maintainer non si aprono issue: si propone la correzione.** Il maintainer è `iCosiSenpai` (ALESSIO COSI), proprietario della repo. Non c'è bisogno di chiederglielo, e non gli si deve far ripetere: `gh auth status` dice quale account GitHub è autenticato e `git remote -v` dice chi possiede la repo — **se coincidono, questa è la postazione del maintainer**. Là un'issue è una notifica a sé stesso, cioè lavoro buttato. Vale invece questa sequenza: (a) la diagnosi con le prove, (b) la patch esatta, con file e righe, (c) chiedere il via per `commit` + `push` su `main`. Il punto (c) non è burocrazia: una modifica **solo locale** viene cancellata da `auto-update.sh` al primo `git fetch`, e non arriva mai ai colleghi — quindi "correggo in locale" equivale a "non correggo". Il divieto di editare `src/`/`scripts/` resta la regola per i colleghi; sul Mac del maintainer è lui a derogare, e la deroga vale per i file che indica.
